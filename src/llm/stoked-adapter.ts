...
/**
 * StokedAdapter - A meta-adapter that enforces strict project management workflows
 * 
 * Capabilities:
 * - Wraps another "backend" adapter (Gemini, Claude, etc.) for raw generation
 * - Enforces "Stoked" workflow rules (PM -> Architect prompt chain)
 * - Manages strict status transitions and workflow adherence
 */
export class StokedAdapter implements LLMAdapter {
    // private _config: LLMConfig; 
    private backendAdapter: LLMAdapter;
    // private _autonomousDataDir: string;
    private verbose: boolean;

    constructor(_config: LLMConfig, backendAdapter: LLMAdapter, _autonomousDataDir: string, verbose: boolean = false) {
        // this._config = config;
        this.backendAdapter = backendAdapter;
        // this._autonomousDataDir = autonomousDataDir;
        this.verbose = verbose;
    }

    // LLMAdapter Interface Implementation
    ...
    async start(options: any): Promise<string> {
        return this.backendAdapter.start(options);
    }

    async stop(instanceId: string): Promise<void> {
        return this.backendAdapter.stop(instanceId);
    }

    async getStatus(instanceId: string): Promise<any> {
        return this.backendAdapter.getStatus(instanceId);
    }

    async isInstalled(): Promise<boolean> {
        return this.backendAdapter.isInstalled();
    }

    async getVersion(): Promise<string | null> {
        return this.backendAdapter.getVersion();
    }

    supportsHooks(): boolean {
        return this.backendAdapter.supportsHooks();
    }

    async installHooks(worktreePath: string, assignmentId: string, sessionId?: string): Promise<void> {
        return this.backendAdapter.installHooks(worktreePath, assignmentId, sessionId);
    }

    async getLastSummary(instanceId: string): Promise<string | null> {
        return this.backendAdapter.getLastSummary(instanceId);
    }

    async prompt(prompt: string): Promise<string> {
        // Pass-through simple prompts
        return this.backendAdapter.prompt(prompt);
    }
    ...
    async complete(
        ...
        // This is where the magic happens for "Project Creation"
        // We can detect if this is a project creation task and intercept it

        if (task.includes("PRODUCT STRATEGY") || task.includes("Project Plan")) {
            // Enforce Stoked Workflow
            return this.handleProjectCreation(task, options?.workingDirectory);
        }

        return this.backendAdapter.complete(task, assignmentId, history);
    }

    // Stoked Specific Methods - The "Skill" of Project Management

    async generateProductStrategy(description: string): Promise<string> {
...
========================`;
    ...
    async generateImplementationPlan(productStrategy: string): Promise<string> {
...
- Any migration or rollout criteria (e.g., "old path fully deprecated", "no data loss during cutover").]`;
    ...
    private async handleProjectCreation(task: string, _workingDir?: string): Promise<string> {
        if (this.verbose) {
            console.log("StokedAdapter: Handling Project Creation via Two-Pass Flow");
        }

        // Pass 1: Product Strategy (PM Persona)
        const strategy = await this.generateProductStrategy(task);

        if (this.verbose) {
            console.log("StokedAdapter: Generated Product Strategy");
        }

        // Pass 2: Implementation Plan (Architect Persona)
        const plan = await this.generateImplementationPlan(strategy);

        if (this.verbose) {
            console.log("StokedAdapter: Generated Implementation Plan");
        }
    ...
    /**
     * Enforce Stoked workflow rules based on current status and signals
     */
    enforceWorkflow(_currentStatus: string, signals: { isComplete: boolean, isBlocked: boolean, isFailed: boolean }): string | null {
        // If work is complete, Stoked workflow mandates "In Review" before "Done"
        // (Generic workflow might go straight to "Dev Complete")
        if (signals.isComplete) {
            return 'In Review';
        }

        // If failed or blocked, we might want specific statuses like "Needs More Info" or "Blocked"
        // rather than just clearing the assignment
        if (signals.isBlocked) {
            return 'Blocked';
        }

        return null; // No status change enforced
    }
}
